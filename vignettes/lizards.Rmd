...
```{r, label="reml_anc_plot_inc", fig.show='hold', fig.height=4, fig.width=7, warning=FALSE, message=FALSE}
library(HDInterval)
inc_int <- hdi(anc_reml) # defaults to 95% HDI
plot(anc_reml, node = c(142, 151), intervals = inc_int, type = "l")
```
The `hdi` function can recover discontinuous intervals
(thanks to the `allowSplit` option, see documentation),
which can be relevent for multi-modal reconstructed densities,
such as node 151 here.
```{r, label="reml_anc_inc_bi", warning=FALSE, message=FALSE}
inc_int[["151"]]
```
...
```{r, label="reml_anc_plot_inc_bad", fig.show='hold', fig.height=4, fig.width=7, warning=FALSE, message=FALSE}
anc_bad <- ancestral(cauchy_reml, n_values = 10) # grid is too coarse
inc_int_bad <- hdi(anc_bad)
plot(anc_bad, node = c(142, 151), intervals = inc_int_bad, type = "l")
```
