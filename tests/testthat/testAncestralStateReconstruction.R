test_that("testLikelihoodTwoTipsTree", {
  ...
  expect_equal(sum(posteriorDensityAncestral(3, 20:25, treerr, trait, start = 0, disp = disp, method = "random.root")), 0.0)
  
  fun <- function(x) posteriorDensityAncestral(3, x, treerr, trait, start = 0, disp = disp, method = "random.root")
...
test_that("testLikelihoodThreeTipsTree", {
  ...
  ## ASR - Errors
  expect_error(posteriorDensityAncestral(1, 1, tree, trait, start = mu, disp = disp, method = "fixed.root"),
               "Ancestral reconstruction is only allowed for ancestral nodes.")
  expect_error(posteriorDensityAncestral(7, 1, tree, trait, start = mu, disp = disp, method = "fixed.root"),
               "This node does not exist in the tree.")
  expect_error(posteriorDensityAncestral(4, 1, tree, trait,start = mu, disp = disp, method = "fixed.root"),
               "Ancestral state reconstruction is not allowed for the root with the fixed root model.")
  expect_error(posteriorDensityAncestral(5, 1, tree, trait,start = NULL, disp = disp, method = "fixed.root"),
               "Starting value must be specified for root node in the `fixed.root` method.")
  expect_error(posteriorDensityAncestral(5, 1, tree, trait,start = NULL, disp = disp, method = "random.root"),
               "Starting value must be specified for root node in the `random.root` method.")
  expect_error(posteriorDensityAncestral(4, 1, tree, trait, start = 0.0, disp = disp, method = "random.root"),
               "In the random root model, the `root.edge` must be non NULL and non zero.")
  expect_error(posteriorDensityAncestral(4, 1, tree, trait, start = 0.0, disp = disp, method = "random.root"),
               "In the random root model, the `root.edge` must be non NULL and non zero.")
  expect_error(posteriorDensityAncestral(4, 1, tree, trait, start = mu, disp = disp, method = "reml"),
               "In the reml model, `start` cannot be specified.")
  
  ## ASR - fixed root
  expect_equal(posteriorDensityAncestral(5, 20, tree, trait, start = mu, disp = disp, method = "fixed.root"), 0.0)
  
  fun <- Vectorize(function(x) {posteriorDensityAncestral(5, x, tree, trait, start = mu, disp = disp, method = "fixed.root")})
  total_mass <- unname(integrate(fun, -Inf, +Inf, rel.tol = .Machine$double.eps^0.5)$value)
  expect_equal(total_mass, 1.0)
  ...
  ## ASR - random root
  expect_equal(posteriorDensityAncestral(5, 20, treerr, trait, start = mu, disp = disp, method = "random.root"), 0.0)
  
  fun <- Vectorize(function(x) {posteriorDensityAncestral(5, x, treerr, trait, start = mu, disp = disp, method = "random.root")})
  total_mass <- unname(integrate(fun, -Inf, +Inf, rel.tol = .Machine$double.eps^0.5)$value)
  expect_equal(total_mass, 1.0)
  
  treerr$root.edge <- 0.001
  fun <- Vectorize(function(x) {-posteriorDensityAncestral(4, x, treerr, trait, start = mu, disp = disp, method = "random.root")})
  MAP <- optim(1.0, fun, method = "Brent", lower = -10, upper = 10)
  expect_equal(MAP$par,  mu, tolerance = 1e-4)
  
  treerr$root.edge <- 100000
  fun <- Vectorize(function(x) {-posteriorDensityAncestral(4, x, treerr, trait, start = 0, disp = disp, method = "random.root")})
  MAP_rr <- optim(1.0, fun, method = "Brent", lower = -10, upper = 10)
  
  ## ASR - reml
  expect_equal(posteriorDensityAncestral(5, 20, tree, trait, start = NULL, disp = disp, method = "reml"), 0.0, tolerance = 1e-5)
  
  fun <- Vectorize(function(x) {posteriorDensityAncestral(4, x, tree, trait, start = NULL, disp = disp, method = "reml")})
  total_mass <- unname(integrate(fun, -Inf, +Inf, rel.tol = .Machine$double.eps^0.5)$value)
  expect_equal(total_mass, 1.0)
...
test_that("testAncestralFit", {
  ...
  expect_equal(unname(as.vector(t(ancestral(fitfr, c(12, 13, 15), c(-0.3, 0.1))))),
               c(posteriorDensityAncestral(12, c(-0.3, 0.1), tree, trait, start = fitfr$x0, disp = fitfr$disp, method = "fixed.root"),
                 posteriorDensityAncestral(13, c(-0.3, 0.1), tree, trait, start = fitfr$x0, disp = fitfr$disp, method = "fixed.root"),
                 posteriorDensityAncestral(15, c(-0.3, 0.1), tree, trait, start = fitfr$x0, disp = fitfr$disp, method = "fixed.root")))
  ...
  expect_equal(unname(as.vector(t(ancestral(fitrr, c(12, 13), c(-0.3, 0.1))))),
               c(posteriorDensityAncestral(12, c(-0.3, 0.1), treerr, trait, start = 0.0, disp = fitrr$disp, method = "random.root"),
                 posteriorDensityAncestral(13, c(-0.3, 0.1), treerr, trait, start = 0.0, disp = fitrr$disp, method = "random.root")))
  ...
  expect_equal(unname(as.vector(t(ancestral(fitreml, c(12, 13), c(-0.3, 0.1))))),
               c(posteriorDensityAncestral(12, c(-0.3, 0.1), tree, trait, start = fitreml$x0, disp = fitreml$disp, method = "reml"),
                 posteriorDensityAncestral(13, c(-0.3, 0.1), tree, trait, start = fitreml$x0, disp = fitreml$disp, method = "reml")))
